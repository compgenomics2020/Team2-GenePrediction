...
def rename_gff(union_input_path, blast_input_path, input_file):
    ...
    output_folder = union_input_path+"ku_union2_gff/"
    ...
    if "ku_union2_gff" in os.listdir(union_input_path):    #if ku_union_gff already exists in union_gff
        if input_file+"_union."+type in os.listdir(output_folder):  #if CGT2049_union.gff already exists in ku_union_gff
            print("Output Directory {} contains file. Please delete it before running the tool for the same file").format(output_folder)
            return False
    else:
        os.mkdir(output_folder)     #make ku_union_gff folder if it does not exist
        output_file = output_folder+"/"+input_file+"_union.gff"   #make file in ku_union_gff folder called CGT2049_union.gff
        subprocess.call(["cp", union_input_file, output_file])      #copy contents from input file to output file (union_gff/CGT2049_union.gff -> union_gff/ku_union_gff/CGT_union_gff)
